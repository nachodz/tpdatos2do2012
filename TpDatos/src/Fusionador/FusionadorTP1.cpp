...
FusionadorTP1::FusionadorTP1(string *filePaths, int tamBuffer, int cant_arch){
...
void FusionadorTP1::destruir(){
	delete[] this->filePaths;
}


int FusionadorTP1::inicializarBuffers(Registro** buffers){
...
void FusionadorTP1::destruirBuffers(Registro** buffers){
...
void FusionadorTP1::abrirArchivos(string filePathsAUsar[], ifstream* archs, int num_archs){
    int i;
...
void FusionadorTP1::cerrarArchivos(ifstream* archs, int num_archs){
    int i;
...
void FusionadorTP1::informar(int cant_archivos, int cant_buffers, int numEtapa){
	ofstream informe;
...
int FusionadorTP1::getCantRegs(){
	return this->cantRegs;
}



Registro FusionadorTP1::minimo(Registro** buffers,unsigned short* v_pos, int num_particiones, unsigned short* pos_min){
...
bool FusionadorTP1::finFusion(Registro** buffers, int num_particiones, unsigned short v_pos[]){
    int i;
...
bool FusionadorTP1::bufferVacio(Registro* buffer, unsigned short pos_actual){
...
void FusionadorTP1::cargarUnBuffer(Registro buffer[], ifstream* arch, int tamBuffer){
    unsigned short i = 0;
    Registro reg_inv;
    reg_inv.ID = MAX_ID;
...
void FusionadorTP1::cargarBuffers(Registro* buffers[], ifstream* archs, int num_particiones, int tamBuffer){
    unsigned short i;
    for(i = 0; i < num_particiones; i++){
...
void FusionadorTP1::fusionar_particiones(ifstream* archs, Registro** buffers, int num_archs, int tamBuffer, string path_nuevo_arch){
...
string FusionadorTP1::IntToStr(int n) {
...
void FusionadorTP1::actualizarPaths(string paths_viejos[], string paths_nuevos[], unsigned short cant_paths){
    unsigned short i;
...
void FusionadorTP1::prepararPaths(string filePathsTotales[],string filePathsAUsar[], int cant, int cant_buffers){
