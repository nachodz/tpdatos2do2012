...
int Fusionador::inicializarBuffers(Ocurrencia** buffers){

    int cont = 0;

    for(int i = 0; i < (this->cant_buffers); i++){
        buffers[i] = new Ocurrencia[this->tamBuffer];
        if(buffers[i] == NULL)
            cont++;
    }
    return cont;
...
void Fusionador::destruirBuffers(Ocurrencia** buffers){
...
Ocurrencia Fusionador::minimo(Ocurrencia** buffers,unsigned short* v_pos, int num_particiones, unsigned short* pos_min){
    unsigned short i = 0, j;
    j = v_pos[0]; // num del registro donde esta parado el buffer 0
    Ocurrencia min = buffers[0][j]; //el primero siempre es el minimo
    unsigned short k = 0; //numero del buffer que tenia el registro minimo

    for(i = 1; i < num_particiones; i++){
        j = v_pos[i];

        if(this->mayor(buffers[i][j],min) == 2){

            min.idTermino = buffers[i][j].idTermino;
            min.idFrase = buffers[i][j].idFrase;
            k = i;
        }
    }
...
bool Fusionador::finFusion(Ocurrencia** buffers, int num_particiones, unsigned short v_pos[]){
    int i;

    for(i = 0; i < num_particiones; i++){
        if (buffers[i][v_pos[i]].idTermino != MAX_ID){ //si el buffer i en la posicion actual tiene MAX_ID
            return false;
        }
    }
    return true;
...
bool Fusionador::bufferVacio(Ocurrencia* buffer, unsigned short pos_actual){
...
void Fusionador::cargarUnBuffer(Ocurrencia buffer[], ifstream* arch, int tamBuffer){
    unsigned short i = 0;
    Ocurrencia reg_inv;
    reg_inv.idTermino = MAX_ID;


    if (!arch->eof()){
		for(i = 0; i < tamBuffer; i++){
			arch->read((char*)&(buffer[i]), sizeof(Ocurrencia));

			if(arch->eof() && (i < tamBuffer - 1)){
				buffer[i].idTermino = reg_inv.idTermino;
				break;
			}
...
void Fusionador::cargarBuffers(Ocurrencia* buffers[], ifstream* archs, int num_particiones, int tamBuffer){
    unsigned short i;
    for(i = 0; i < num_particiones; i++){
...
void Fusionador::fusionar_particiones(ifstream* archs, Ocurrencia** buffers, int num_archs, int tamBuffer, string path_nuevo_arch){
    ...
    while(!finFusion(buffers, num_archs, v_pos)){
        ...
        nuevo_arch.write((char*)&reg_min, sizeof(Ocurrencia));
        this->cantRegs++;
        nuevo_arch.flush();
...
int Fusionador::merge(){
    ...
    Ocurrencia* buffers[this->cant_buffers];
...
int Fusionador::mayor(Ocurrencia ocurrencia1, Ocurrencia ocurrencia2) {
